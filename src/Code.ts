...
const UNTIL_DATE = 5;
const KEYWORD = 'メーデー';
const CH_NATIONAL_GEOGRAPHIC = '142_65406';
...
function updateSheet() {
  ...
  const currentTvPrograms: TvProgram[] = sheet
    .getRange('A2:D')
    .getValues()
    .map((it) => ({
  ...
  // curl -s 'https://tvguide.myjcom.jp/api/mypage/get_searchresult/' -F 'keyword=メーデー！' -F 'channel=142_65406' -F 'offset=0' | jq
  const fetchedTvPrograms: TvProgram[] = fetchTvPrograms(KEYWORD, CH_NATIONAL_GEOGRAPHIC, 3);
  ...
  // ヘッダーを追加
  sheet.getRange(1, 1, headValues.length, headValues[0].length).setValues(headValues).setHorizontalAlignment('center');
  ...
  const afterTodayFilter = SpreadsheetApp.newFilterCriteria().whenDateAfter(SpreadsheetApp.RelativeDate.TODAY).build();
...
function notifyToGmail(spreadsheet: Spreadsheet, unrecordedValues: SheetValue[]) {
  ...
  const subject = '『メーデー！』を録画せよ！';

  const textBody = `${spreadsheet.getName()} ${spreadsheet.getUrl()}`;

  const htmlBody =
    unrecordedValues.map((value) => `<a href="${value[3]}">${value[0]}</a>`).join('<br>') +
